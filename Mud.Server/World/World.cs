...
public class World : IWorld, IAreaManager, IRoomManager, ICharacterManager, IItemManager, IAuraManager, IQuestManager
{
    ...
    public QuestBlueprint? GetQuestBlueprint(int id)
        => GetBlueprintById(_questBlueprints, id);
    ...
    public AreaBlueprint? GetAreaBlueprint(int id)
        => GetBlueprintById(_areaBlueprints, id);
    ...
    public RoomBlueprint? GetRoomBlueprint(int id)
        => GetBlueprintById(_roomBlueprints, id);
    ...
    public IRoom AddRoom(Guid guid, RoomBlueprint blueprint, IArea area)
    {
        ...
        var room = new Room.Room(Guid.NewGuid(), blueprint, area);
        room.Recompute();
        _rooms.Add(room);
        return room;
    }
    ...
    public CharacterBlueprintBase? GetCharacterBlueprint(int id)
        => GetBlueprintById(_characterBlueprints, id);
    ...
    {
        var character = new Character.PlayableCharacter.PlayableCharacter(guid, playableCharacterData, player, room);
    ...
    {
        ArgumentNullException.ThrowIfNull(blueprint);
        var character = new Character.NonPlayableCharacter.NonPlayableCharacter(guid, blueprint, room);
        _characters.Add(character);
        character.Recompute();
        room.Recompute();
        return character;
    ...
    {
        ArgumentNullException.ThrowIfNull(blueprint);
        var character = new Character.NonPlayableCharacter.NonPlayableCharacter(guid, blueprint, petData, room);
        _characters.Add(character);
        character.Recompute();
        room.Recompute();
        return character;
    ...
    public ItemBlueprintBase? GetItemBlueprint(int id)
        => GetBlueprintById(_itemBlueprints, id);
    ...
    public IItemCorpse AddItemCorpse(Guid guid, IRoom room, ICharacter victim)
    {
        ...
        var item = new ItemCorpse(guid, blueprint, room, victim);
        _items.Add(item);
        item.Recompute();
        return item;
    }
    ...
    {
        var blueprint = GetItemBlueprint<ItemCorpseBlueprint>(Settings.CorpseBlueprintId) ?? throw new Exception($"Corpse blueprint {Settings.CorpseBlueprintId} not found");
        var item = new ItemCorpse(guid, blueprint, room, victim, killer);
        _items.Add(item);
        item.Recompute();
        return item;
    }
    ...
    public IItemMoney? AddItemMoney(Guid guid, long silverCoins, long goldCoins, IContainer container)
    {
        ...
        var money = new ItemMoney(guid, blueprint, silverCoins, goldCoins, container);
        _items.Add(money);
        return money;
    }
    ...
    {
        var aura = new Aura.Aura(abilityName, source, auraFlags, level, duration, affects);
    ...
    {
        var aura = new Aura.Aura(auraData);
        target.AddAura(aura, recompute);
        return aura;
    }
    ...
    private void FixItems()
    {
        ...
        foreach (var itemBlueprint in ItemBlueprints.OrderBy(x => x.Id))
        {
    ...
    private void FixResets()
    {
        ...
        foreach (var room in _rooms.Where(x => x.Blueprint.Resets?.Count > 0).OrderBy(x => x.Blueprint.Id))
        {
            ...
            foreach (var reset in room.Blueprint.Resets)
            {
                switch (reset)
                {
                    ...
                    {
                        // check wear location
                        var blueprint = GetItemBlueprint(itemInEquipmentReset.ItemId);
                        if (blueprint != null)
                        {
