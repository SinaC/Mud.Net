...
public class RaceManager : IRaceManager
{
    ...
    public RaceManager(IAssemblyHelper assemblyHelper, IAbilityManager abilityManager)
    {
        ...
        _races = assemblyHelper.AllReferencedAssemblies.SelectMany(a => a.GetTypes()
            ...
            .Select(t => CreateInstance(t, iPlayableRaceType.IsAssignableFrom(t), abilityManager))) // don't use DependencyContainer
            .Where(r => r != null)!
            .ToList()!;
    }
    ...
    public IRace? this[string name]
        => _races.FirstOrDefault(x => StringCompareHelpers.StringEquals(x.Name, name));

    #endregion

    private static IRace? CreateInstance(Type type, bool isPlayableRace, IAbilityManager abilityManager)
    {
        if (isPlayableRace)
