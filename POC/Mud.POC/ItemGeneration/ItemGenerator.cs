...
namespace Mud.POC.ItemGeneration;
...
public class ItemGenerator
{
    ...
    public IItem? Generate(int level, IContainer container)
    {
        // select random blueprint id in available range
        var blueprintId = RandomManager.Random(_blueprintIds);
        // create item
        var item = ItemManager.AddItem(Guid.NewGuid(), blueprintId, "ItemGenerator", container);
        if (item == null)
            return null;
        // enchant item
        Enchant(item, level);
    ...
    public void Enchant(IItem item, int level)
    {
        ...
        // TODO: depending on item type:
        // armor: ac = -10*level * rarity / 2
        // weapon: dice count = level * rarity / 2
        //         dice value = level * rarity / 2
        if (item is IItemWeapon weapon)
        {
            var diceCount = level * RarityMultiplierFactor100(rarity) / 200;
            var diceValue = level * RarityMultiplierFactor100(rarity) / 200;
