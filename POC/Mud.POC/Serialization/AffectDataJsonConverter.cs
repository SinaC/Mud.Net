...
public class AffectDataJsonConverter
{
    private readonly Dictionary<string, Type> _discriminatorTypeMap;
    private readonly HashSet<Type> _affectDataTypes;
    private readonly JsonSerializerOptions _options;

    public AffectDataJsonConverter()
    {
        ...
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            ...
            {
                var attribute = type.GetCustomAttribute<AffectDataDiscriminatorAttribute>();
                if (attribute != null)
                    _discriminatorTypeMap.Add(attribute.Discriminator ?? type.Name, type);
            }
        }

        _affectDataTypes = [.. _discriminatorTypeMap.Values.Distinct()];
    ...
    public object Deserialize(string json)
    {
        ...
        if (obj is not null && obj.RootElement.TryGetProperty("$discriminator", out JsonElement property) && TryGetType(property.GetString() ?? "", out var type) && type is not null && CanConvert(type))
    ...
    private bool CanConvert(Type typeToConvert)
        => _affectDataTypes.Contains(typeToConvert);
    ...
    {
        var attribute = type.GetCustomAttribute<AffectDataDiscriminatorAttribute>() ?? throw new ArgumentException("Discriminator not found on the type.");
