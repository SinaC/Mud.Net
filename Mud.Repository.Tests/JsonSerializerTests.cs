using DeepEqual.Syntax;
using Mud.Domain;
using Mud.Domain.SerializationData.Account;
using Mud.Domain.SerializationData.Avatar;
using Mud.Server.Item;
using System.Text.Json;

namespace Mud.Repository.Tests
{
    [TestClass]
    public class JsonSerializerTests : MappingTestsBase
    {
        [TestMethod]
        public void SerializeThenDeserialize_Avatar()
        {
            var avatar = new AvatarData
            {
                Version = 1,
                AccountName = "account",
                CreationTime = DateTime.Now,
                RoomId = 0,
                SilverCoins = 0,
                GoldCoins = 0,
                Wimpy = 0,
                Experience = 0,
                Alignment = 0,
                Trains = 0,
                Practices = 0,
                AutoFlags = AutoFlags.Sacrifice | AutoFlags.Affect,
                CurrentQuests = [],
                LearnedAbilities = [],
                LearnedAbilityGroups = [],
                Conditions = [],
                Aliases = [],
                Cooldowns = [],
                Pets = [],
                // CharacterBaseData
                Name = "Player",
                Race = "Human",
                Class = "Warrior",
                Level = 1,
                Sex = Sex.Male,
                Size = Sizes.Medium,
                CurrentResources = new Dictionary<ResourceKinds, int>
                {
                    { ResourceKinds.HitPoints, 1 },
                    { ResourceKinds.MovePoints, 1 },
                },
                MaxResources = new Dictionary<ResourceKinds, int>
                {
                    { ResourceKinds.HitPoints, 1 },
                    { ResourceKinds.MovePoints, 1 },
                },
                Currencies = new Dictionary<Currencies, int>
                {
                    { Currencies.QuestPoints, 1 }
                },
                Equipments = [],
                Inventory = [],
                Auras = [],
                CharacterFlags = "Blind,DetectMagic",
                Immunities = "Fire,Pouet",
                Resistances = "Weapon,Wood",
                Vulnerabilities = string.Empty,
                ShieldFlags = string.Empty,
                Attributes = [],
            };

            var serialized = JsonSerializer.Serialize(avatar, _options);
            var deserialized = JsonSerializer.Deserialize<AvatarData>(serialized, _options)!;

            avatar.WithDeepEqual(deserialized).Assert();
        }

        [TestMethod]
        public void Deserialize_Avatar()
        {
            var deserialized = JsonSerializer.Deserialize<AvatarData>(AvatarFileContent, _options)!;

            Assert.IsNotNull(deserialized);
            Assert.AreEqual("glouk", deserialized.Name);
            Assert.IsEmpty(deserialized.Aliases);
            Assert.HasCount(7, deserialized.CurrentResources);
            Assert.HasCount(7, deserialized.MaxResources);
            Assert.ContainsSingle(deserialized.Pets);
            Assert.HasCount(4, deserialized.Equipments);
            Assert.ContainsSingle(deserialized.Equipments.Where(x => x.Item is ItemLightData));
            Assert.HasCount(2, deserialized.Equipments.Where(x => x.Item is ItemWeaponData));
            Assert.HasCount(2, deserialized.Inventory);
            Assert.HasCount(13, deserialized.Auras);
            Assert.HasCount(12, deserialized.Attributes);
        }

        [TestMethod]
        public void Deserialize_Account()
        {
            var deserialized = JsonSerializer.Deserialize<AccountData>(AccountFileContent, _options)!;

            Assert.IsNotNull(deserialized);
            Assert.AreEqual("sinac", deserialized.Username);
            Assert.HasCount(2, deserialized.Aliases);
            Assert.IsNotNull(deserialized.AdminData);
            Assert.AreEqual(AdminLevels.Implementor, deserialized.AdminData.AdminLevel);
            Assert.AreEqual(Enum.GetValues<WiznetFlags>().Where(x => x != WiznetFlags.Resets).Aggregate(WiznetFlags.None, (acc, f) => acc | f), deserialized.AdminData.WiznetFlags);
            Assert.HasCount(3, deserialized.AvatarMetaDatas);
        }

        private const string AvatarFileContent =
        @"{
  ""Version"": 1,
  ""AccountName"": ""sinac"",
  ""CreationTime"": ""2025-11-11T16:31:33.7841739+01:00"",
  ""RoomId"": 3001,
  ""SilverCoins"": 109,
  ""GoldCoins"": 5000,
  ""Wimpy"": 0,
  ""Experience"": 120000,
  ""Alignment"": 0,
  ""Trains"": 22,
  ""Practices"": 89,
  ""AutoFlags"": 127,
  ""Currencies"": {},
  ""PulseLeftBeforeNextAutomaticQuest"": 0,
  ""CurrentQuests"": [],
  ""LearnedAbilities"": [
    {
      ""Name"": ""Cat Form"",
      ""Level"": 15,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Fourth Wield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Third Wield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Axe"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Dagger"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Dual Wield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Flail"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Mace"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Polearm"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Spear"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Staff(weapon)"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Sword"",
      ""Level"": 10,
      ""Learned"": 85,
      ""Rating"": 1
    },
    {
      ""Name"": ""Whip"",
      ""Level"": 10,
      ""Learned"": 1,
      ""Rating"": 1
    },
    {
      ""Name"": ""Test"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Fire Shield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Ice Shield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Shadow Word: Pain"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Shock Shield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Construct"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Giant Size"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Testroom"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Acid Blast"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Armor"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Bless"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Blindness"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Burning Hands"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Call Lightning"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Calm"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cancellation"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cause Critical"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cause Light"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cause Serious"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Chain Lightning"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Change Sex"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Charm Person"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Chill Touch"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Colour Spray"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Continual Light"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Control Weather"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Create Food"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Create Rose"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Create Spring"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Create Water"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Blindness"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Critical"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Disease"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Light"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Poison"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Cure Serious"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Curse"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Demonfire"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Evil"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Good"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Hidden"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Invis"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Magic"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Detect Poison"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Dispel Evil"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Dispel Good"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Dispel Magic"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Earthquake"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Enchant Armor"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Enchant Weapon"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Energy Drain"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Faerie Fire"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Faerie Fog"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Farsight"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Fireball"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Fireproof"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Flamestrike"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Floating Disc"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Fly"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Frenzy"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Gate"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Giant Strength"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Harm"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Haste"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Heal"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Heat Metal"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Holy Word"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Identify"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Infravision"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Invisibility"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Lightning Bolt"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Locate Object"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Magic Missile"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Mass Healing"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Mass Invis"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Nexus"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Pass Door"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Plague"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Poison"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Portal"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Protection Evil"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Protection Good"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Ray of Truth"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Recharge"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Refresh"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Remove Curse"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Sanctuary"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Shield"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Shocking Grasp"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Sleep"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Slow"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Stone Skin"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Summon"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Teleport"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Ventriloquate"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Weaken"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Word of Recall"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""General Purpose"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""High Explosive"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Acid Breath"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Fire Breath"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Frost Breath"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Gas Breath"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Lightning Breath"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Know Alignment"",
      ""Level"": 1,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""bear form"",
      ""Level"": 5,
      ""Learned"": 100,
      ""Rating"": 0
    },
    {
      ""Name"": ""Claw"",
      ""Level"": 15,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Rake"",
      ""Level"": 20,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Maul"",
      ""Level"": 20,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Demoralizing Roar"",
      ""Level"": 10,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Ferocious Bite"",
      ""Level"": 20,
      ""Learned"": 100,
      ""Rating"": 1
    },
    {
      ""Name"": ""Swipe"",
      ""Level"": 25,
      ""Learned"": 100,
      ""Rating"": 1
    }
  ],
  ""LearnedAbilityGroups"": [
    {
      ""Name"": ""druid basics"",
      ""Cost"": 0
    }
  ],
  ""Conditions"": {
    ""Drunk"": 0,
    ""Full"": 2,
    ""Thirst"": 25,
    ""Hunger"": 25
  },
  ""Aliases"": {},
  ""Cooldowns"": {},
  ""Pets"": [
    {
      ""BlueprintId"": 3093,
      ""Description"": ""The rottweiler looks like a strong, fierce fighter.\r\nA neck tag says \u0027I belong to Fnar\u0027.\r\n"",
      ""Name"": ""toto"",
      ""Race"": ""dog"",
      ""Class"": """",
      ""Level"": 7,
      ""Sex"": 0,
      ""Size"": 2,
      ""CurrentResources"": {
        ""HitPoints"": 120,
        ""MovePoints"": 1000,
        ""Mana"": 150,
        ""Psy"": 0,
        ""Energy"": 0,
        ""Rage"": 0,
        ""Combo"": 0
      },
      ""MaxResources"": {
        ""HitPoints"": 120,
        ""MovePoints"": 1000,
        ""Mana"": 150,
        ""Psy"": 0,
        ""Energy"": 0,
        ""Rage"": 0,
        ""Combo"": 0
      },
      ""Equipments"": [],
      ""Inventory"": [],
      ""Auras"": [
        {
          ""AbilityName"": ""Haste"",
          ""Level"": 7,
          ""PulseLeft"": -1,
          ""AuraFlags"": 4,
          ""Affects"": [
            {
              ""$type"": ""characterAttribute"",
              ""Operator"": 0,
              ""Location"": 4,
              ""Modifier"": 1
            },
            {
              ""$type"": ""characterFlags"",
              ""Operator"": 1,
              ""Modifier"": ""Haste""
            },
            {
              ""$type"": ""CharacterAdditionalHitAffectData"",
              ""AdditionalHitCount"": 1
            }
          ]
        }
      ],
      ""CharacterFlags"": ""DetectHidden,charm"",
      ""Immunities"": """",
      ""Resistances"": ""Magic"",
      ""Vulnerabilities"": """",
      ""ShieldFlags"": """",
      ""Attributes"": {
        ""Strength"": 12,
        ""Intelligence"": 12,
        ""Wisdom"": 12,
        ""Dexterity"": 12,
        ""Constitution"": 12,
        ""SavingThrow"": 0,
        ""HitRoll"": 3,
        ""DamRoll"": 7,
        ""ArmorBash"": 40,
        ""ArmorPierce"": 40,
        ""ArmorSlash"": 40,
        ""ArmorExotic"": 80
      }
    }
  ],
  ""Name"": ""glouk"",
  ""Race"": ""insectoid"",
  ""Class"": ""druid"",
  ""Level"": 49,
  ""Sex"": 0,
  ""Size"": 2,
  ""CurrentResources"": {
    ""HitPoints"": 15800,
    ""MovePoints"": 1110,
    ""Mana"": 5298,
    ""Psy"": 100,
    ""Energy"": 100,
    ""Rage"": 75,
    ""Combo"": 0
  },
  ""MaxResources"": {
    ""HitPoints"": 21800,
    ""MovePoints"": 2110,
    ""Mana"": 31600,
    ""Psy"": 100,
    ""Energy"": 100,
    ""Rage"": 100,
    ""Combo"": 0
  },
  ""Equipments"": [
    {
      ""Slot"": 1,
      ""Item"": {
        ""$type"": ""light"",
        ""TimeLeft"": -1,
        ""ItemId"": 21,
        ""Level"": 0,
        ""DecayPulseLeft"": 0,
        ""ItemFlags"": ""Glowing"",
        ""Auras"": []
      }
    },
    {
      ""Slot"": 4,
      ""Item"": {
        ""ItemId"": 2114,
        ""Level"": 10,
        ""DecayPulseLeft"": 0,
        ""ItemFlags"": """",
        ""Auras"": [
          {
            ""AbilityName"": ""Test"",
            ""Level"": 1,
            ""PulseLeft"": 1184,
            ""AuraFlags"": 2,
            ""Affects"": [
              {
                ""$type"": ""itemFlags"",
                ""Operator"": 0,
                ""Modifier"": ""Glowing,Humming,Magic""
              },
              {
                ""$type"": ""characterAttribute"",
                ""Operator"": 0,
                ""Location"": 14,
                ""Modifier"": -490
              },
              {
                ""$type"": ""characterAttribute"",
                ""Operator"": 0,
                ""Location"": 6,
                ""Modifier"": 49
              }
            ]
          }
        ]
      }
    },
    {
      ""Slot"": 13,
      ""Item"": {
        ""$type"": ""weapon"",
        ""WeaponFlags"": """",
        ""ItemId"": 3350,
        ""Level"": 16,
        ""DecayPulseLeft"": 0,
        ""ItemFlags"": ""Magic"",
        ""Auras"": [
          {
            ""AbilityName"": ""Envenom"",
            ""Level"": 1,
            ""PulseLeft"": 308,
            ""AuraFlags"": 2,
            ""Affects"": [
              {
                ""$type"": ""weaponFlags"",
                ""Operator"": 1,
                ""Modifier"": ""Poison""
              }
            ]
          }
        ]
      }
    },
    {
      ""Slot"": 13,
      ""Item"": {
        ""$type"": ""weapon"",
        ""WeaponFlags"": ""Vorpal,TwoHands"",
        ""ItemId"": 3005,
        ""Level"": 42,
        ""DecayPulseLeft"": 0,
        ""ItemFlags"": ""Humming,Bless"",
        ""Auras"": []
      }
    }
  ],
  ""Inventory"": [
    {
      ""ItemId"": 2111,
      ""Level"": 10,
      ""DecayPulseLeft"": 0,
      ""ItemFlags"": """",
      ""Auras"": []
    },
    {
      ""ItemId"": 2104,
      ""Level"": 3,
      ""DecayPulseLeft"": 0,
      ""ItemFlags"": """",
      ""Auras"": []
    }
  ],
  ""Auras"": [
    {
      ""AbilityName"": ""Giant Strength"",
      ""Level"": 1,
      ""PulseLeft"": 9576,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 1,
          ""Modifier"": 4
        }
      ]
    },
    {
      ""AbilityName"": ""Detect Evil"",
      ""Level"": 1,
      ""PulseLeft"": 9772,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""DetectEvil""
        }
      ]
    },
    {
      ""AbilityName"": ""Detect Good"",
      ""Level"": 1,
      ""PulseLeft"": 9804,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""DetectGood""
        }
      ]
    },
    {
      ""AbilityName"": ""Detect Invis"",
      ""Level"": 1,
      ""PulseLeft"": 9820,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""DetectInvis""
        }
      ]
    },
    {
      ""AbilityName"": ""Detect Hidden"",
      ""Level"": 1,
      ""PulseLeft"": 9876,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""DetectHidden""
        }
      ]
    },
    {
      ""AbilityName"": ""Shock Shield"",
      ""Level"": 1,
      ""PulseLeft"": 9576,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""ShockShield""
        }
      ]
    },
    {
      ""AbilityName"": ""Fire Shield"",
      ""Level"": 1,
      ""PulseLeft"": 9376,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""FireShield""
        }
      ]
    },
    {
      ""AbilityName"": ""Ice Shield"",
      ""Level"": 1,
      ""PulseLeft"": 9176,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""IceShield""
        }
      ]
    },
    {
      ""AbilityName"": ""Berserk"",
      ""Level"": 1,
      ""PulseLeft"": 4576,
      ""AuraFlags"": 2,
      ""Affects"": [
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""Berserk""
        },
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 9,
          ""Modifier"": 9
        },
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 10,
          ""Modifier"": 9
        },
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 14,
          ""Modifier"": 90
        }
      ]
    },
    {
      ""AbilityName"": ""Protection Evil"",
      ""Level"": 22,
      ""PulseLeft"": 5312,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 8,
          ""Modifier"": -1
        },
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""ProtectEvil""
        },
        {
          ""$type"": ""noData"",
          ""AffectName"": ""ProtectEvil""
        }
      ]
    },
    {
      ""AbilityName"": ""Protection Good"",
      ""Level"": 33,
      ""PulseLeft"": 5356,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 8,
          ""Modifier"": -1
        },
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""ProtectGood""
        },
        {
          ""$type"": ""noData"",
          ""AffectName"": ""ProtectGood""
        }
      ]
    },
    {
      ""AbilityName"": ""Sanctuary"",
      ""Level"": 36,
      ""PulseLeft"": 1560,
      ""AuraFlags"": 0,
      ""Affects"": [
        {
          ""$type"": ""shieldFlags"",
          ""Operator"": 1,
          ""Modifier"": ""Sanctuary""
        },
        {
          ""$type"": ""noData"",
          ""AffectName"": ""Sanctuary""
        }
      ]
    },
    {
      ""AbilityName"": ""Haste"",
      ""Level"": 49,
      ""PulseLeft"": -1,
      ""AuraFlags"": 4,
      ""Affects"": [
        {
          ""$type"": ""characterAttribute"",
          ""Operator"": 0,
          ""Location"": 4,
          ""Modifier"": 4
        },
        {
          ""$type"": ""characterFlags"",
          ""Operator"": 1,
          ""Modifier"": ""Haste""
        },
        {
          ""$type"": ""CharacterAdditionalHitAffectData"",
          ""AdditionalHitCount"": 1
        }
      ]
    }
  ],
  ""CharacterFlags"": ""Haste"",
  ""Immunities"": """",
  ""Resistances"": ""Acid,Bash,Disease,Poison,Slash"",
  ""Vulnerabilities"": ""Cold,Fire,Pierce"",
  ""ShieldFlags"": """",
  ""Attributes"": {
    ""Strength"": 22,
    ""Intelligence"": 22,
    ""Wisdom"": 22,
    ""Dexterity"": 22,
    ""Constitution"": 24,
    ""SavingThrow"": 0,
    ""HitRoll"": 0,
    ""DamRoll"": 0,
    ""ArmorBash"": 100,
    ""ArmorPierce"": 100,
    ""ArmorSlash"": 100,
    ""ArmorExotic"": 100
  }
}";

        private const string AccountFileContent =
            @"{
  ""Version"": 1,
  ""Username"": ""sinac"",
  ""Password"": ""password"",
  ""PagingLineCount"": 0,
  ""Aliases"": {
    ""pouet"": ""who"",
    ""cda"": ""commanddebug admin""
  },
  ""AdminData"":
  {
    ""Version"": 1,
    ""AdminLevel"": 7,
    ""WiznetFlags"": 61439
  },
  ""AvatarMetaDatas"":
  [
    {
      ""Name"": ""sinac"",
      ""Version"": 1,
      ""Level"": 46,
      ""Class"": ""Druid"",
      ""Race"": ""Insectoid"",
      ""RoomId"": 3001
    },
    {
      ""Name"": ""glouk"",
      ""Version"": 1,
      ""Level"": 49,
      ""Class"": ""Druid"",
      ""Race"": ""Insectoid"",
      ""RoomId"": 3001
    },
    {
      ""Name"": ""fnar"",
      ""Version"": 1,
      ""Level"": 7,
      ""Class"": ""Warrior"",
      ""Race"": ""Giant"",
      ""RoomId"": 3001
    }
  ]
}";
    }
}
