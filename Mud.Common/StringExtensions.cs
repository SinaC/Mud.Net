...
namespace Mud.Common
{
    public static class StringExtensions
    {
        ...
        public static string ToPascalCase(this string s) => string.Join(" ", s.Split(' ').Select(token => (token == "of" || token == "in" || token == "on" || token == "to") ? token : char.ToUpperInvariant(token[0]) + token.Substring(1)));
