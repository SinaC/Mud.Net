...
public static class TableGenerators
{
    ...
    public static readonly Lazy<TableGenerator<IAbilityLearned>> LearnedAbilitiesTableGenerator = new(() =>
    {
        ...
        generator.AddColumn("Cost", 23, x =>
        {
            if (x.Learned == 0)
                return "n/a";
            if (x.AbilityUsage.AbilityDefinition.Type == AbilityTypes.Passive)
                return "%m%passive%x%";
            else if (x.AbilityUsage.AbilityDefinition.Type == AbilityTypes.Weapon)
                return "%y%weapon%x%";
            if (x.HasCost)
            {
                var sb = new StringBuilder();
                var shortDisplay = x.AbilityUsage.ResourceCosts.Length > 1;
                for (var i = 0; i < x.AbilityUsage.ResourceCosts.Length; i++)
                {
                    ...
                    switch (cost.CostAmountOperator)
                    {
                        ...
                        case CostAmountOperators.AllWithMin:
                            sb.Append($"min {cost.CostAmount} {cost.ResourceKind.ResourceColor(shortDisplay)}");
                            break;
                        default:
                            sb.Append("???");
                            break;
                    }
                    if (i < x.AbilityUsage.ResourceCosts.Length - 1)
                        sb.Append(", ");
        ...
        generator.AddColumn("Type", 10, x => x.AbilityUsage.AbilityDefinition.Type.ToString());
        generator.AddColumn("Cooldown", 10, x => x.AbilityUsage.AbilityDefinition.CooldownInSeconds.HasValue ? TimeSpan.FromSeconds(x.AbilityUsage.AbilityDefinition.CooldownInSeconds.Value).FormatDelayShort() : "---");
        return generator;
    });
    ...
    public static readonly Lazy<TableGenerator<IClass>> ClassTableGenerator = new(() =>
    {
        ...
        generator.AddColumn("Resource(s)", 24, x => string.Join(",", (x.ResourceKinds ?? Enumerable.Empty<ResourceKinds>()).Select(x => x.ResourceColor(false))));
    ...
    public static readonly Lazy<TableGenerator<IAbilityDefinition>> FullInfoAbilityTableGenerator = new(() =>
    {
        ...
        generator.AddColumn("Cooldown", 10, x => x.CooldownInSeconds.HasValue ? TimeSpan.FromSeconds(x.CooldownInSeconds.Value).FormatDelayShort() : "---");
    ...
    public static readonly Lazy<TableGenerator<IAbilityUsage>> FullInfoAbilityUsageTableGenerator = new(() =>
    {
        ...
        generator.AddColumn("Resource", 10,
            x =>
            {
                ...
                {
                    var sb = new StringBuilder();
                    var shortDisplay = x.ResourceCosts.Length > 1;
                    for (var i = 0; i < x.ResourceCosts.Length; i++)
                    {
