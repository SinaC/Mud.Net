...
namespace Mud.Server.StateMachine;
...
[Export(typeof(IAvatarCreationStateMachine))]
public class AvatarCreationStateMachine : InputTrapBase<IPlayer, AvatarCreationStates>, IAvatarCreationStateMachine
{
    ...
    public AvatarCreationStateMachine(ILogger logger, IServerPlayerCommand serverPlayerCommand, IRaceManager raceManager, IClassManager classManager, IUniquenessManager uniquenessManager, ITimeManager timeManager, IRoomManager roomManager, IGameActionManager gameActionManager, IParser parser, IAbilityGroupManager abilityGroupManager)
    {
        Logger = logger;
...
help <name>  help on skills and groups, or other help topics
            ...
            else
            {
                ...
                {
                    GameActionManager.Execute<Help, IAdmin>(admin, parameter);
                }
                else
                {
                    GameActionManager.Execute<Help, IPlayer>(player, parameter);
                }
            }
        }
        else if (command == "info" || command == "groups")
        {
            ...
            else
            {
                ...
                var abilityGroupDefinition = AbilityGroupManager.Search(Parser.ParseParameter(parameter));
    ...
    private AvatarCreationStates ProcessImmediateImpersonate(IPlayer player, string input)
    {
        ...
        if (input == "y" || input == "yes")
        {
            ...
            if (player is IAdmin admin)
            {
                executeResult = GameActionManager.Execute<Commands.Admin.Avatar.Impersonate, IAdmin>(admin, _name);
            }
            else
            {
                executeResult = GameActionManager.Execute<Commands.Player.Avatar.Impersonate, IPlayer>(player, _name);
            }
