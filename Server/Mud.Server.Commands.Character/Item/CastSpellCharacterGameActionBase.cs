    ...
    where TCharacterGameActionInfo : class, ICharacterGameActionInfo
{
    ...
    protected CastSpellCharacterGameActionBase(ILogger<CastSpellCharacterGameActionBase<TCharacter, TCharacterGameActionInfo>> logger, IParser parser, IAbilityManager abilityManager)
    {
        Logger = logger;
        Parser = parser;
    ...
    protected string? CastSpell(IItem item, string spellName, int spellLevel)
    {
        ...
        var spellActionInput = new SpellActionInput(abilityDefinition, Actor, spellLevel, new CastFromItemOptions { Item = item }, Parser.NoParameters);
