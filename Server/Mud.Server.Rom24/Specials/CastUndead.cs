using Microsoft.Extensions.Logging;
using Mud.Domain;
using Mud.Random;
using Mud.Server.Domain.Attributes;
using Mud.Server.Interfaces.Character;
using Mud.Server.Interfaces.Special;

namespace Mud.Server.Rom24.Specials;

[SpecialBehavior("spec_cast_undead")]
public class CastUndead : ISpecialBehavior
{
    private ILogger<CastAdept> Logger { get; }
    private IRandomManager RandomManager { get; }

    public CastUndead(ILogger<CastAdept> logger, IRandomManager randomManager)
    {
        Logger = logger;
        RandomManager = randomManager;
    }

    // use harmful spells on victim
    public bool Execute(INonPlayableCharacter npc)
    {
        if (!npc.IsValid || npc.Room == null
            || npc.Position <= Positions.Sleeping || npc.Fighting == null)
            return false;

        // search a random 'victim'
        var victim = RandomManager.Random(npc.Room.People.Where(x => npc.Fighting == x && RandomManager.Chance(25)));
        if (victim == null)
            return false;

        string? spellName = null;
        int minNpcLevel = 0;

        for (; ; )
        {
            var choice = RandomManager.Next(16);
            switch (choice)
            {
                case 0:
                    spellName = "curse";
                    minNpcLevel = 0;
                    break;
                case 1:
                    spellName = "weaken";
                    minNpcLevel = 3;
                    break;
                case 2:
                    spellName = "chill touch";
                    minNpcLevel = 6;
                    break;
                case 3:
                    spellName = "blindness";
                    minNpcLevel = 9;
                    break;
                case 4:
                    spellName = "poison";
                    minNpcLevel = 12;
                    break;
                case 5:
                    spellName = "energy drain";
                    minNpcLevel = 15;
                    break;
                case 6:
                    spellName = "harm";
                    minNpcLevel = 18;
                    break;
                case 7:
                    spellName = "teleport";
                    minNpcLevel = 21;
                    break;
                case 8:
                    spellName = "plague";
                    minNpcLevel = 20;
                    break;
                default:
                    spellName = "harm";
                    minNpcLevel = 18;
                    break;
            }
            if (npc.Level >= minNpcLevel)
                break; // spell found
        }
        return CastSpell(npc, spellName, victim);
    }

    private bool CastSpell(INonPlayableCharacter caster, string spellName, ICharacter victim)
    {
        var successfull = caster.CastSpell(spellName, victim);
        if (!successfull)
            Logger.LogError("CastUndead: error on {caster} while casting spell {spellName} on {victimName}", caster.DebugName, spellName, victim.DebugName);
        return successfull;
    }
}
