...
{
    [SpecialBehavior("spec_ogre_member")]
    public class OgreMember : ISpecialBehavior
    {
        private IRandomManager RandomManager { get; }
        private DangerousNeighborhood Options { get; }

        public OgreMember(IRandomManager randomManager,  IOptions<Rom24Options> rom24Options)
        {
        ...
        public bool Execute(INonPlayableCharacter npc)
        {
            ...
            var victims = npc.Room.People.OfType<INonPlayableCharacter>().Where(victim => victim != npc && victim.Blueprint.Group == Options.Trolls && npc.Level > victim.Level - 2 && victim.IsSafe(npc) == null).ToList();
            if (victims.Count == 0)
                return false;
            ...
            var message = RandomManager.Range(0, 6) switch
            {
                ...
                2 => "{0:N} says 'What's slimy Ogre trash like you doing around here?'",
